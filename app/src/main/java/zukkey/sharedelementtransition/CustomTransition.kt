...
class CustomTransition : Transition() {
  ...
  override fun createAnimator(sceneRoot: ViewGroup?, startValues: TransitionValues?, endValues: TransitionValues?): Animator {
    val startBounds = startValues?.values?.get(PROPNAME_IMAGE) as Rect
    val endBounds = endValues?.values?.get(PROPNAME_IMAGE) as Rect
    val isStart = endBounds.width() > startBounds.width() && endBounds.height() > startBounds.height()
